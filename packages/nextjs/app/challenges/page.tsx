...
const ChallengesPage = () => {
  ...
  const fetchChallengesAndStats = async () => {
    console.log("Starting fetchChallengesAndStats...");
    try {
      setIsLoading(true);
      
      // Test connection first
      console.log("Testing web3 connection...");
      const connectionTest = await web3Service.testConnection();
      console.log("Connection test result:", connectionTest);
      
      if (!connectionTest) {
        console.warn("Web3 connection failed, using empty data");
        setChallenges([]);
        setPlatformStats({
          totalChallenges: 0,
          totalProofs: 0,
      ...
      console.log("Raw challenges from blockchain:", realChallenges);
      console.log("Number of challenges found:", realChallenges.length);
      
      // Fetch platform stats
      console.log("Fetching platform stats...");
      const stats = await web3Service.getPlatformStats();
      console.log("Platform stats:", stats);
      setPlatformStats(stats);
      
      const transformedChallenges: Challenge[] = realChallenges.map((challenge, index) => {
        console.log(`Transforming challenge ${index + 1}:`, challenge);
        
        // Map challenge type from number to string
        const getChallengeTypeString = (typeEnum: number): "community" | "custom" | "community_service" => {
          const typeMap = {
            0: "community",
            1: "custom", 
            2: "community_service",
            3: "community_service", // social_challenge maps to community_service for UI
            4: "community_service"  // storage_incentive maps to community_service for UI
          };
        ...
        return {
          ...
          creatorEns: challenge.ensSubdomain, // Note: it's ensSubdomain in web3Service
          stakeAmount: challenge.stakeAmount,
          deadline: challenge.deadline,
          maxParticipants: challenge.maxParticipants || 100, // Default if not set
          currentParticipants: challenge.participantCount || 0,
          challengeType: getChallengeTypeString(challenge.challengeType),
          likes: Math.floor(Math.random() * 50),
          comments: Math.floor(Math.random() * 20),
          isLiked: false,
          proofRequired: challenge.ipfsMetadata || "Photo or video evidence required",
          status: challenge.status === 0 ? "active" : challenge.status === 1 ? "ended" : "active", // 0 = active, 1 = ended
        };
      });
      ...
      console.error("Failed to fetch challenges and stats:", error);
      // Set fallback data
      setChallenges([]);
      setPlatformStats({
        totalChallenges: 0,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link
              ...
              className="flex items-center space-x-2 px-6 py-3 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white font-semibold rounded-xl transition-all duration-200 shadow-lg hover:shadow-xl group"
            >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {[
              { 
                ...
                value: platformStats ? platformStats.totalChallenges : challenges.length, 
                icon: RocketLaunchIcon, 
                color: "from-blue-500 to-cyan-500" 
              },
              { 
                label: "Total Proofs", 
                value: platformStats ? platformStats.totalProofs : "...", 
                icon: UserGroupIcon, 
                color: "from-green-500 to-emerald-500" 
              },
              { 
                label: "Total Staked", 
                value: platformStats ? `${parseFloat(platformStats.totalRewardsDistributed).toFixed(2)} ETH` : "...", 
                icon: TrophyIcon, 
                color: "from-yellow-500 to-orange-500" 
              },
              { 
                label: "Active Users", 
                value: platformStats ? platformStats.totalUsers : "...", 
                icon: SparklesIcon, 
        ...
        <div className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm border border-white/50 dark:border-slate-700/50 rounded-2xl p-6 mb-8 shadow-lg">
          ...
          <div className="flex items-center space-x-3 mb-4">
            ...
            <button
              onClick={fetchChallengesAndStats}
              disabled={isLoading}
